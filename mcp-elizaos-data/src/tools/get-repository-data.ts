...
// Input schema
export const GetRepositoryDataArgsSchema = z.object({
...
const RepositoryDataSchema = z.object({
  ...
  open_issues: z.number().optional().describe("Number of open issues"),
  language: z.string().optional().describe("Primary programming language"),
  created_at: z.string().optional().describe("Repository creation date"),
  updated_at: z.string().optional().describe("Last update date"),
  contributors: z.array(ContributorSchema).optional().describe("Repository contributors"),
  topics: z.array(z.string()).optional().describe("Repository topics/tags"),
});

const RepositoryDataResponseSchema = z.object({
  total_repositories: z.number().describe("Total number of repositories"),
  repositories: z.array(RepositoryDataSchema).describe("Array of repository data"),
  updated_at: z.string().describe("When the data was last updated"),
});

export type GetRepositoryDataArgs = z.infer<typeof GetRepositoryDataArgsSchema>;
export type RepositoryDataResponse = z.infer<typeof RepositoryDataResponseSchema>;

// Tool schema for MCP
export const getRepositoryDataSchema = {
  ...
  inputSchema: GetRepositoryDataArgsSchema,
};

export async function getRepositoryDataHandler(args: any) {
  const validatedArgs = GetRepositoryDataArgsSchema.parse(args);
  ...
  try {
    ...
    if (!result) {
        ...
        repositories: [
          {
            ...
            contributors: validatedArgs.include_contributors ? [
      ...
      if (validatedArgs.repository_name) {
        const filteredRepos = mockData.repositories.filter(r => 
          r.name.toLowerCase().includes(validatedArgs.repository_name!.toLowerCase()) ||
          r.full_name.toLowerCase().includes(validatedArgs.repository_name!.toLowerCase())
        );
      ...
      if (!validatedArgs.include_stats) {
    ...
    if (!validatedArgs.include_stats && processedData.repositories) {
    ...
    if (!validatedArgs.include_contributors && processedData.repositories) {
