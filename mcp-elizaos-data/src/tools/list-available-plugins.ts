...
// Input schema
export const ListAvailablePluginsArgsSchema = z.object({
...
const PluginInfoSchema = z.object({
  ...
  repository: z.string().optional().describe("Plugin repository URL"),
  documentation: z.string().optional().describe("Documentation URL"),
  dependencies: z.array(z.string()).optional().describe("Plugin dependencies"),
  keywords: z.array(z.string()).optional().describe("Plugin keywords"),
  last_updated: z.string().optional().describe("Last update timestamp"),
  status: z.enum(["active", "deprecated", "experimental"]).optional().describe("Plugin status"),
});

const PluginListResponseSchema = z.object({
  total_plugins: z.number().describe("Total number of plugins"),
  plugins: z.array(PluginInfoSchema).describe("Array of plugin information"),
  categories: z.array(z.string()).describe("Available plugin categories"),
  updated_at: z.string().describe("When the data was last updated"),
});

export type ListAvailablePluginsArgs = z.infer<typeof ListAvailablePluginsArgsSchema>;
export type PluginListResponse = z.infer<typeof PluginListResponseSchema>;

// Tool schema for MCP
export const listAvailablePluginsSchema = {
  ...
  inputSchema: ListAvailablePluginsArgsSchema,
};

export async function listAvailablePluginsHandler(args: any) {
  const validatedArgs = ListAvailablePluginsArgsSchema.parse(args);
  ...
  try {
    ...
    if (!result) {
      ...
      if (validatedArgs.category) {
        filteredPlugins = mockPlugins.filter(p => 
          p.category?.toLowerCase() === validatedArgs.category!.toLowerCase()
        );
      }

      // Sort plugins
      const sortField = validatedArgs.sort_by;
      ...
      if (!validatedArgs.include_details) {
    ...
    if (validatedArgs.category && processedData.plugins) {
      // Filter by category if specified
    ...
    if (processedData.plugins && validatedArgs.sort_by) {
      const sortField = validatedArgs.sort_by;
